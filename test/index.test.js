...
describe('tailwindcss-animations plugins', () => {
  ...
  it('use a bezier curve as a timing function animation', async () => {
    ...
    expect(css).toMatch(
      '.animate-bezier-sine-in{animation-timing-function:cubic-bezier(0.12,0,0.39,0)}'
    );
  });
  ...
  it('use a custom bezier curve as a timing function animation', async () => {
    const css = await generatePluginCSS({
      content:
        '<div class="animate-bezier-[cubic-bezier(0,0,0,0)]">Hello</div>',
    });
  ...
  it('use predefined animation steps', async () => {
    const css = await generatePluginCSS({
  ...
  it('use custom animation steps', async () => {
    const css = await generatePluginCSS({
